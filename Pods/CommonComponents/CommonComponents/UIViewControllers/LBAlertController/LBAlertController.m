...
-(void)loadView{
    ...
    _alertTitleLabel.frame = CGRectMake((CGRectGetWidth(self.view.frame)-alertTitleWidth)/2, 20, alertTitleWidth, [_alertTitleLabel sizeThatFits:CGSizeMake(alertTitleWidth, MAXFLOAT)].height);
    [self.view addSubview:_alertTitleLabel];
    
    
    CGFloat alertMessageWidth = CGRectGetWidth(self.view.bounds)-25*2;
    _alertMessageLabel.frame = CGRectMake((CGRectGetWidth(self.view.frame)-alertMessageWidth)/2, CGRectGetMaxY(_alertTitleLabel.frame)+20, alertMessageWidth, [_alertMessageLabel sizeThatFits:CGSizeMake(alertMessageWidth, CGFLOAT_MAX)].height);
    ...
    if (horizontalSpace < 0) {//button竖排模式
        ...
        [_buttonArray enumerateObjectsUsingBlock:^(LBAlertActionButton * _Nonnull actionBtn, NSUInteger idx, BOOL * _Nonnull stop) {
            actionBtn.frame = CGRectMake((CGRectGetWidth(self.view.bounds)-CGRectGetWidth(actionBtn.bounds))/2, (CGRectGetMaxY(weakSelf.alertMessageLabel.frame)+CGRectGetHeight(weakSelf.userView.frame)+30)+heightCurrentSum+idx*10, CGRectGetWidth(actionBtn.bounds), CGRectGetHeight(actionBtn.bounds));
        ...
        [_buttonArray enumerateObjectsUsingBlock:^(LBAlertActionButton * _Nonnull actionBtn, NSUInteger idx, BOOL * _Nonnull stop) {
            actionBtn.frame = CGRectMake(widthCurrentSum+horizontalSpace*(idx+1), CGRectGetMaxY(weakSelf.alertMessageLabel.frame)+CGRectGetHeight(weakSelf.userView.frame)+30, CGRectGetWidth(actionBtn.bounds), CGRectGetHeight(actionBtn.bounds));
            [self.view  addSubview:actionBtn];
    ...
    contentViewFrame.size.height = CGRectGetMaxY(_buttonArray.lastObject.frame);
